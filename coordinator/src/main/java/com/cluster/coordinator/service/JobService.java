...
public class JobService {
    ...
    @Transactional
    public Job createJob(String name, String taskType, String input, int requestedCpu, long requestedMemory) {
        // 1. Validate request
        ...
        JobPartitioner partitioner = partitioners.stream()
                .filter(p -> p.supports(taskType))
                .findFirst()
                .orElseThrow(() -> new IllegalArgumentException("Unsupported taskType: " + taskType));
        ...
        log.info("Created Job {} (type={}) with {} partitions", job.getId(), taskType, job.getTotalPartitions());
        return job;
    }

...
import com.cluster.coordinator.service.partitioner.SumRangeJobPartitioner;
...
public class JobServiceTest {
    ...
    public void testValidSumRangeJob() { // A, F, H, I, J, M
        Job job = jobService.createJob("Test Job", "SUM_RANGE", "{\"start\":1,\"end\":12000000}", 3, 3072L);
    ...
    public void testStartGreaterThanEnd() { // E
        Exception exception = assertThrows(IllegalArgumentException.class, () -> {
            jobService.createJob("Test Job", "SUM_RANGE", "{\"start\":100,\"end\":1}", 1, 1024L);
        });
    ...
    public void testUnevenDivision() { // G
        Job job = jobService.createJob("Uneven Job", "SUM_RANGE", "{\"start\":1,\"end\":10}", 3, 1024L);
        List<Task> tasks = jobService.getTasksForJob(job.getId());
        assertEquals(3, tasks.size());
    ...
    public void testLargeWorkload() { // K
        ...
        Job job = jobService.createJob("Large Job", "SUM_RANGE", "{\"start\":1,\"end\":250000000}", 1, 1024L);
    ...
    public void testTransactionalJobCreationFailure() { // L
        ...
        assertThrows(IllegalArgumentException.class, () -> {
            jobService.createJob("Fail Job", "SUM_RANGE", "{\"start\":100,\"end\":1}", 1, 1024L);
        });

...
public class TaskExecutionServiceTest {
    ...
    private Task createTask(
            String id, String jobId, int partitionId, int cpu, long memory, TaskState state) {
        Task t = new Task();
        t.setId(id);
        t.setJobId(jobId);
    ...
    public void testC_RunningToFailed() {
        ...
        assertEquals(
                ...
                taskAssignmentRepository.findByTaskId("t1").orElseThrow().getState());
        assertEquals(4, resourceReservationService.getAvailableCpu("w1"));
    ...
    public void testM_N_O_WrongIdentityRejected() {
        ...
        assertEquals(
                TaskState.ASSIGNED,
                taskRepository.findById("t1").orElseThrow().getState()); // rejected
    ...
    public void testU_MultiPartitionJob_CompletesProperly() {
        ...
        assertEquals(
                ...
                jobRepository
                        .findById("job1")
                        .orElseThrow()
                        .getState()); // Wait, it's RUNNING because dispatch transitioned it

        TaskResultMessage msg2 =
                TaskResultMessage.builder()
                        .taskId("t2")
                        .jobId("job1")
                        .partitionId(2)

...
export function WorkerTable({ workers, onWorkerClick }: WorkerTableProps) {
  ...
  const sorted = [...workers].sort((a, b) => {
    ...
    if (sortKey === "hostname") {
      ...
      aVal = a.cpuUsagePercent;
      bVal = b.cpuUsagePercent;
    } else {
      aVal = a.memoryUsagePercent;
      bVal = b.memoryUsagePercent;
    }
          ...
          {sorted.map((worker) => {
            ...
            return (
              ...
              >
                <td className="py-3 px-4">
                  <div>
                    <p className="text-sm font-semibold text-slate-200">
                      {worker.hostname} &middot; {worker.id.substring(0, 8)}
                    </p>
                  ...
                  {isOffline ? (
                    ...
                    <div>
                      <p className="text-xs text-slate-400 mb-1">
                        {Math.round(
                          (worker.cpuCores * (worker.cpuUsagePercent || 0)) / 100,
                        )}
                        /{worker.cpuCores} cores
                      </p>
                      <MiniBar
                        value={worker.cpuUsagePercent || 0}
                        max={100}
                        color={
                          (worker.cpuUsagePercent || 0) > 85
                            ? "bg-red-500"
                            : (worker.cpuUsagePercent || 0) > 60
                              ? "bg-amber-500"
                              : "bg-cyan-500"
                        }
                      />
                ...
                <td className="py-3 px-4">
                  {isOffline ? (
                    ...
                    <div>
                      <p className="text-xs text-slate-400 mb-1">
                        {Math.round(
                          (worker.memoryRamMb * (worker.memoryUsagePercent || 0)) /
                            100000,
                        )}
                        /{Math.round(worker.memoryRamMb / 1024)} GB
                      </p>
                      <MiniBar
                        value={worker.memoryUsagePercent || 0}
                        max={100}
                        color={
                          (worker.memoryUsagePercent || 0) > 85
                            ? "bg-red-500"
                            : (worker.memoryUsagePercent || 0) > 60
                              ? "bg-amber-500"
                              : "bg-violet-500"
                        }
                      />
                ...
                <td className="py-3 px-4">
                  <span
                    className={`text-xs font-mono ${isOffline ? "text-red-400" : "text-slate-400"}`}
                  >
                    {isOffline
                      ? `${Math.round(hbAge / 60)}m ago`
                      : `${hbAge}s ago`}
                  </span>
                </td>
              </tr>

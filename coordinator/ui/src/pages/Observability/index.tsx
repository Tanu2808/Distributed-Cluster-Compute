...
function MetricBox({
  ...
  return (
    ...
    >
      ...
      <p
        className={`text-xl font-bold ${highlight ? "gradient-text" : "text-slate-100"}`}
      >
        {value}
      </p>
...
export default function Observability() {
  ...
  const gpuPct =
    (s?.totalGpus ?? 0) > 0
      ? Math.round(((s?.usedGpus ?? 0) / (s?.totalGpus ?? 1)) * 100)
      : 0;
        ...
        <div className="space-y-8">
          ...
          <div className="relative card overflow-hidden">
            ...
            <div className="relative px-8 py-10 text-center">
              <p className="section-title mb-3 tracking-[0.3em]">
              ...
              <p className="text-slate-500 text-sm mb-8">
                All physical workers aggregated as one compute unit
              </p>
          ...
          <div className="card p-6">
            <SectionHeader
              ...
              subtitle="Physical PCs → Logical Cluster"
            />
            ...
            <div className="flex flex-col items-center">
              ...
              <div className="w-48 rounded-xl border border-accent-500/40 bg-accent-500/5 px-5 py-3 text-center mb-2 relative shadow-glow-cyan">
                <p className="text-xs font-semibold uppercase tracking-wider text-accent-400 mb-1">
                  Logical Cluster
                </p>
                <p className="text-sm font-bold text-slate-100">
                  {s.totalCpuCores} CPU · {s.totalMemoryGb} GB
                </p>
                <p className="text-xs text-slate-500">
                  {s.totalGpus} GPU · {s.totalStorageTb} TB
                </p>
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-[11px] text-slate-400">
                        {offline ? "—" : `${w.cpuCores} CPU`}
                      </p>
                      <p className="text-[11px] text-slate-400">
                        {offline
                          ? "—"
                          : `${Math.round(w.memoryRamMb / 1024)} GB`}
                      </p>
                      <p className="text-[11px] text-slate-600">
                        {w.gpuCount > 0 ? `${w.gpuCount} GPUs` : "No GPU"}
                      </p>
                      {offline && (
                        <p className="text-[10px] text-red-400 mt-1">Offline</p>
                      )}
                    </div>
                  );
                })}
          ...
          <div className="card p-6">
            <SectionHeader
              icon={<Cpu size={16} />}
              title="CPU"
              subtitle="Aggregate across all workers"
            />
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
              <MetricBox
                label="Total Cores"
                value={s.totalCpuCores}
                highlight
              />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div>
                ...
                <div className="flex items-center justify-center h-[140px] border border-slate-800 rounded-lg text-slate-500 text-sm">
          ...
          <div className="card p-6">
            <SectionHeader
              ...
              title="Memory"
              subtitle="Aggregate RAM across all workers"
            />
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div>
                ...
                <p className="section-title mb-3">Utilization History</p>
                <div className="flex items-center justify-center h-[120px] border border-slate-800 rounded-lg text-slate-500 text-sm">
                  No historical metrics available
                </div>
              </div>
              <div className="flex flex-col items-center justify-center gap-4">
                ...
                />
                <div>
                  <p className="section-title mb-2 text-center">
                    Per-Worker RAM
                  </p>
                  <BarChartWidget
                    data={memContrib}
          ...
          <div className="card p-6">
            <SectionHeader
              icon={<Zap size={16} />}
              title="GPU"
              subtitle="Aggregate GPU availability"
            />
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
            ...
            <div className="space-y-2">
              ...
              {workers.map((w) => (
                <div
                  key={w.id}
                  className="flex items-center gap-4 p-3 rounded-lg bg-surface-800 border border-slate-800"
                >
                  <div
                  ...
                  {w.gpuCount > 0 && (
                    <>
                      <div className="text-right">
                        ...
                        <p
                          className={`text-sm font-semibold ${w.state === "OFFLINE" ? "text-slate-600" : "text-emerald-400"}`}
                        >
                  ...
                  {w.gpuCount === 0 && (
                    <p className="text-xs text-slate-600">Not equipped</p>
                  )}
                </div>
              ))}
            </div>
          ...
          <div className="card p-6">
            <SectionHeader
              ...
              title="Storage"
              subtitle="Aggregate logical storage — each worker's disk contributes independently"
            />
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
              <MetricBox
                label="Total Storage"
                value={`${s.totalStorageTb} TB`}
                highlight
              />
              <MetricBox
                label="Used"
                value={`${s.usedStorageTb.toFixed(1)} TB`}
              />
              <MetricBox
                label="Available"
                value={`${s.availableStorageTb.toFixed(1)} TB`}
              />
              <MetricBox
                label="Utilization"
            ...
            <div className="progress-bar h-3 mb-2">
              <div
                className="progress-fill bg-amber-500 h-3"
                style={{ width: `${storagePct}%` }}
              />
            </div>
            <p className="text-xs text-slate-600">
              Note: Aggregate storage represents the sum of individual worker
              disks, not a shared filesystem.
            </p>
          </div>
          ...
          <div className="card p-6">
            <SectionHeader
              ...
              title="Network"
              subtitle="Aggregate inbound/outbound across all workers"
            />
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
              <MetricBox
                label="Inbound"
                value={`${s.networkInboundMbps} Mbps`}
                highlight
              />
              <MetricBox
                label="Outbound"
                value={`${s.networkOutboundMbps} Mbps`}
              />
              <MetricBox
                label="Throughput"
                value={`${s.networkInboundMbps + s.networkOutboundMbps} Mbps`}
              />

...
export async function fetchSettings(): Promise<ClusterSettings> {
  ...
  return {
    coordinator: {
      hostname:
        map["coordinator.hostname"] || mockSettings.coordinator.hostname,
      port: parseInt(map["coordinator.port"]) || mockSettings.coordinator.port,
      clusterId:
        map["coordinator.clusterId"] || mockSettings.coordinator.clusterId,
      clusterName:
        map["coordinator.clusterName"] || mockSettings.coordinator.clusterName,
    },
    workerRegistration: {
      registrationMode:
        (map["workerRegistration.registrationMode"] as RegistrationMode) ||
        mockSettings.workerRegistration.registrationMode,
      requireAuthentication: parseBool(
        map["workerRegistration.requireAuthentication"],
        mockSettings.workerRegistration.requireAuthentication,
      ),
      heartbeatIntervalSeconds:
        parseInt(map["workerRegistration.heartbeatIntervalSeconds"]) ||
        mockSettings.workerRegistration.heartbeatIntervalSeconds,
      workerTimeoutSeconds:
        parseInt(map["workerRegistration.workerTimeoutSeconds"]) ||
        mockSettings.workerRegistration.workerTimeoutSeconds,
    },
    resourceConfig: {
      maxCpuAllocationPercent:
        parseInt(map["resourceConfig.maxCpuAllocationPercent"]) ||
        mockSettings.resourceConfig.maxCpuAllocationPercent,
      maxMemoryAllocationPercent:
        parseInt(map["resourceConfig.maxMemoryAllocationPercent"]) ||
        mockSettings.resourceConfig.maxMemoryAllocationPercent,
      gpuAllocationEnabled: parseBool(
        map["resourceConfig.gpuAllocationEnabled"],
        mockSettings.resourceConfig.gpuAllocationEnabled,
      ),
      perWorkerCpuLimit:
        parseInt(map["resourceConfig.perWorkerCpuLimit"]) ||
        mockSettings.resourceConfig.perWorkerCpuLimit,
      perWorkerMemoryLimitGb:
        parseInt(map["resourceConfig.perWorkerMemoryLimitGb"]) ||
        mockSettings.resourceConfig.perWorkerMemoryLimitGb,
    },
    monitoring: {
      metricsCollectionIntervalSeconds:
        parseInt(map["monitoring.metricsCollectionIntervalSeconds"]) ||
        mockSettings.monitoring.metricsCollectionIntervalSeconds,
      eventRetentionDays:
        parseInt(map["monitoring.eventRetentionDays"]) ||
        mockSettings.monitoring.eventRetentionDays,
      logLevel:
        (map["monitoring.logLevel"] as LogLevel) ||
        mockSettings.monitoring.logLevel,
      enableMetricsExport: parseBool(
        map["monitoring.enableMetricsExport"],
        mockSettings.monitoring.enableMetricsExport,
      ),
    },
  };
}

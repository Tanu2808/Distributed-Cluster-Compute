...
export function StatusBadge({ status, size = 'md' }: StatusBadgeProps) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full border font-medium ${sizeClass} ${classes}`}>
      <span className={`rounded-full ${status === 'ONLINE' ? 'animate-pulse' : ''} ${

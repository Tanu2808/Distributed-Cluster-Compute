...
export function useClusterSummary() {
  ...
  const cluster = useMemo(() => {
    ...
    if (!clusterStatusData) return undefined;
  ...
  }, [clusterStatusData, workers]);

  return {
    data: cluster,
    isLoading: statusLoading || workersLoading,
    isError: statusError || workersError,
    error: statusErr || workersErr,
    refetch: () => {
      refetchStatus();
      refetchWorkers();
    },

...
const ALL_STATUSES: WorkerStatus[] = [
  "ONLINE",
  "BUSY",
  "OFFLINE",
  "UNHEALTHY",
...
export default function Nodes() {
  ...
  return (
    <div className="p-6 space-y-5 animate-fade-in">
      {isError && (
        <ErrorBanner
          message="Failed to load workers from the coordinator."
          onRetry={() => refetch()}
        />
      )}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          {
            label: "Online",
            value: (counts.ONLINE ?? 0) + (counts.BUSY ?? 0),
            color: "text-green-400",
          },
          { label: "Busy", value: counts.BUSY ?? 0, color: "text-amber-400" },
          {
            label: "Offline",
            value: (counts.OFFLINE ?? 0) + (counts.UNHEALTHY ?? 0),
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <select
            value={filterStatus}
            onChange={(e) =>
              setFilterStatus(e.target.value as WorkerStatus | "ALL")
            }

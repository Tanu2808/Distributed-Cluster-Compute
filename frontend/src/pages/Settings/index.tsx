...
const tabs: { id: SettingsTab; label: string; icon: React.ReactNode }[] = [
  ...
  { id: 'registration', label: 'Worker Registration',  icon: <Users size={14} /> },
        ...
        <div className="card p-6">
          ...
          {activeTab === 'coordinator' && (
            <>
              ...
              />
              <FieldGroup label="Cluster Name" description="Human-readable name for this cluster">
                <input
              ...
              <FieldGroup label="Cluster ID" description="Unique identifier (auto-generated)">
                <input
              ...
              <FieldGroup label="Hostname" description="Coordinator bind hostname or IP">
                <input
              ...
              <FieldGroup label="Port" description="Coordinator API and WebSocket port">
                <input
                  className="input-field w-32 font-mono"
                  type="number"
                  value={settings.coordinator.port}
                  onChange={e => updateCoord({ port: parseInt(e.target.value) || 8080 })}
                />
              </FieldGroup>
            </>
          )}
          ...
          {activeTab === 'registration' && (
            <>
              ...
              />
              <FieldGroup label="Registration Mode" description="How workers are allowed to register">
                <select
                  className="select-field w-48"
                  value={settings.workerRegistration.registrationMode}
                  onChange={e => updateReg({ registrationMode: e.target.value as RegistrationMode })}
                >
              ...
              <FieldGroup label="Require Authentication" description="Enforce auth before a worker can join">
                <Toggle
                  checked={settings.workerRegistration.requireAuthentication}
                  onChange={v => updateReg({ requireAuthentication: v })}
                  label={settings.workerRegistration.requireAuthentication ? 'Enabled' : 'Disabled'}
                />
              </FieldGroup>
              <FieldGroup label="Heartbeat Interval" description="How often workers send a heartbeat (seconds)">
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={e => updateReg({ heartbeatIntervalSeconds: parseInt(e.target.value) || 10 })}
                  />
                  <span className="text-xs text-slate-500">seconds</span>
                </div>
              </FieldGroup>
              <FieldGroup label="Worker Timeout" description="Time before a silent worker is marked offline">
                <div className="flex items-center gap-2">
                  <input
                    className="input-field w-24 font-mono"
                    type="number"
                    ...
                    onChange={e => updateReg({ workerTimeoutSeconds: parseInt(e.target.value) || 30 })}
                  />
                  <span className="text-xs text-slate-500">seconds</span>
                </div>
              </FieldGroup>
          ...
          {activeTab === 'resources' && (
            <>
              ...
              />
              <FieldGroup label="Max CPU Allocation" description="Maximum CPU % any task can consume per worker">
                <div className="flex items-center gap-3">
                  <input
                    type="range"
                    min={10} max={100}
                    value={settings.resourceConfig.maxCpuAllocationPercent}
                    onChange={e => updateRes({ maxCpuAllocationPercent: parseInt(e.target.value) })}
                    className="w-40 accent-cyan-400"
                  />
                  <span className="text-sm font-mono text-slate-300 w-10">{settings.resourceConfig.maxCpuAllocationPercent}%</span>
                </div>
              </FieldGroup>
              <FieldGroup label="Max Memory Allocation" description="Maximum memory % per worker for task execution">
                <div className="flex items-center gap-3">
                  <input
                    type="range"
                    min={10} max={100}
                    value={settings.resourceConfig.maxMemoryAllocationPercent}
                    onChange={e => updateRes({ maxMemoryAllocationPercent: parseInt(e.target.value) })}
                    className="w-40 accent-violet-400"
                  />
                  <span className="text-sm font-mono text-slate-300 w-10">{settings.resourceConfig.maxMemoryAllocationPercent}%</span>
                </div>
              </FieldGroup>
              <FieldGroup label="GPU Allocation" description="Allow tasks to request GPU resources">
                <Toggle
                  checked={settings.resourceConfig.gpuAllocationEnabled}
                  onChange={v => updateRes({ gpuAllocationEnabled: v })}
                  label={settings.resourceConfig.gpuAllocationEnabled ? 'Enabled' : 'Disabled'}
                />
              </FieldGroup>
              <FieldGroup label="Per-Worker CPU Limit" description="Max cores per worker for task assignment (0 = unlimited)">
                <div className="flex items-center gap-2">
                  <input
                    className="input-field w-24 font-mono"
                    type="number"
                    ...
                    onChange={e => updateRes({ perWorkerCpuLimit: parseInt(e.target.value) || 0 })}
                  />
                  <span className="text-xs text-slate-500">cores (0 = no limit)</span>
                </div>
              </FieldGroup>
              <FieldGroup label="Per-Worker Memory Limit" description="Max GB per worker for task assignment (0 = unlimited)">
                <div className="flex items-center gap-2">
                  <input
                    className="input-field w-24 font-mono"
                    type="number"
                    ...
                    onChange={e => updateRes({ perWorkerMemoryLimitGb: parseInt(e.target.value) || 0 })}
                  />
                  <span className="text-xs text-slate-500">GB (0 = no limit)</span>
                </div>
              </FieldGroup>
            </>
          )}
          ...
          {activeTab === 'monitoring' && (
            <>
              ...
              />
              <FieldGroup label="Metrics Interval" description="How often the coordinator collects metrics from workers">
                <div className="flex items-center gap-2">
                  <input
                    className="input-field w-24 font-mono"
                    type="number"
                    ...
                    onChange={e => updateMon({ metricsCollectionIntervalSeconds: parseInt(e.target.value) || 5 })}
                  />
                  <span className="text-xs text-slate-500">seconds</span>
                </div>
              </FieldGroup>
              <FieldGroup label="Event Retention" description="How long cluster events are stored">
                <div className="flex items-center gap-2">
                  <input
                    className="input-field w-24 font-mono"
                    type="number"
                    ...
                    onChange={e => updateMon({ eventRetentionDays: parseInt(e.target.value) || 30 })}
                  />
                  <span className="text-xs text-slate-500">days</span>
                </div>
              </FieldGroup>
              <FieldGroup label="Log Level" description="Verbosity of coordinator logs">
                <select
                  className="select-field w-36"
                  value={settings.monitoring.logLevel}
                  onChange={e => updateMon({ logLevel: e.target.value as LogLevel })}
                >
              ...
              <FieldGroup label="Metrics Export" description="Export metrics to an external monitoring system">
                <Toggle
                  checked={settings.monitoring.enableMetricsExport}
                  onChange={v => updateMon({ enableMetricsExport: v })}
                  label={settings.monitoring.enableMetricsExport ? 'Enabled' : 'Disabled'}
                />
              </FieldGroup>
            </>
          )}

...
public class ResourceAdmissionService {
    ...
    private final ConcurrentHashMap<String, ResourceReservation> reservations =
    ...
    public synchronized boolean tryReserve(
            WorkerTask task, SystemMetrics currentMetrics, WorkerConfig.Execution executionConfig) {
        ...
        boolean admitted =
                canAdmit(task, allocatedMem, allocatedCpu, currentMetrics, executionConfig);
        if (admitted) {
            reservations.put(
                    task.getTaskId(),
            ...
            log.info(
                    "Reserved resources for task {}: {} MB RAM, {} CPU cores. Total allocated: {}"
                            + " MB, {} cores",
                    task.getTaskId(),
                    task.getRequiredMemoryMb(),
                    task.getRequiredCpuCores(),
    ...
    public synchronized boolean release(String taskId) {
        ...
        if (removed != null) {
            log.info(
                    "Released resources for task {}: {} MB RAM, {} CPU cores. Total allocated: {}"
                            + " MB, {} cores",
                    taskId,
                    removed.getMemoryMb(),
                    removed.getCpuCores(),
    ...
    public boolean canAdmit(
            ...
            WorkerConfig.Execution executionConfig) {

  ...
  return (
    <div className={`grid ${colClass} gap-x-6 gap-y-2.5 text-xs ${className}`}>
      {items.map((item, idx) => (
        <div
          key={idx}
          className="flex items-baseline justify-between gap-4 py-1 border-b border-console-borderSubtle"
        >
          <span className="text-console-textMuted font-medium shrink-0">
            {item.label}
          </span>
          <span
            className={`text-console-text text-right truncate ${item.mono ? "font-mono text-[11px]" : ""}`}
          >

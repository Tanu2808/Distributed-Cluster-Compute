  ...
  const handleConfirmReset = async () => {
    ...
    try {
      ...
      setNotification({
        type: "success",
        message:
          "Settings reset to default values. Cluster configuration cleared.",
      });
      await refetch();
      setTimeout(() => {
        navigate("/setup");
            ...
            items={[
              {
                ...
                value: workerSettings?.workerId ? (
                  <button
                    type="button"
                    onClick={() =>
                      handleCopy(workerSettings.workerId, "workerId")
                    }
              ...
              },
              {
                ...
                mono: true,
                value:
                  workerSettings?.version !== undefined
                    ? `v${workerSettings.version}`
                    : "Unavailable",
              },
              {
                label: "Registration",
        ...
        <ConsoleCard title="Cluster Settings">
          <KeyValueTable
            ...
            items={[
              {
                label: "Cluster Name",
                value: clusterSettings?.clusterName || (
              ...
              },
              {
                ...
                value: clusterSettings?.clusterId ? (
                  <button
                    type="button"
                    onClick={() =>
                      handleCopy(clusterSettings.clusterId, "clusterId")
                    }
                    className="inline-flex items-center gap-1.5 hover:text-console-accent text-console-text transition-colors text-left"
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-xs text-console-textDim leading-relaxed max-w-2xl">
              Clears saved cluster connection parameters and transitions this
              worker back to initial setup. The unique Worker ID is preserved.
            </p>
          </div>
